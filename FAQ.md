...
**Q: I said "no" to the repo-vault registration prompt — can I change my mind?**
A: Yes. Edit `~/.config/obsidian-memory/repos.json` and either flip `enabled` to `true`, or delete the entry to surface the registration prompt again next session. `python3 "$CLAUDE_PLUGIN_ROOT/scripts/_repos.py" register <repo-path> --enabled --project <name>` does the same thing from the CLI.

**Q: What's the difference between the personal vault and a repo-vault?**
A: The personal vault (`$OBSIDIAN_VAULT_PATH`, default `~/Documents/Obsidian Vault`) is yours alone — cross-project memory like tools, preferences, and learnings that aren't tied to one repo. Each opted-in project repo can also act as a repo-vault: its own `decisions/`, `learnings/`, `references/` subdirs (when they exist) become writable destinations for save-memory, and all its frontmattered `.md` files are searchable alongside the personal vault. The repo-vault is committed by you (whenever you commit the repo); the personal vault auto-commits at SessionEnd.
...
**Q: Why doesn't the repo-vault auto-commit?**
A: Two reasons. (1) The personal vault is yours; the repo-vault is your project's git history, which often has stricter conventions (commit messages, signed commits, PR review, CI). (2) save-memory writes during a session, and the natural review boundary is whenever you'd commit anyway — the working-tree change is visible in your `git status` until you decide what to do with it.

**Q: Can a repo-vault note link to a personal-vault note (or vice versa)?**
