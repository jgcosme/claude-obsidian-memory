...
A: Yes. From inside the repo, run `/obsidian-memory:project enable` — that flips the registry entry to enabled and runs the frontmatter backfill. Without a Claude session, edit `~/.config/obsidian-memory/projects.json` and either flip `enabled` to `true` or delete the entry to surface the registration prompt again next session. `python3 "$CLAUDE_PLUGIN_ROOT/scripts/_projects.py" register <repo-path> --enabled --project <name>` does the same thing from the CLI.
