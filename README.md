...
- **Frontmatter as source of truth.** Every note declares `type`, `description`, `created_at`, `created_by`, `updated_at`, `updated_by` (and `project` when scoped). The plugin writes those fields automatically and derives recall, summaries, and audits from them — there is no separate index to maintain. The frontmatter doubles as a description-level overview (path + type + one-line hook for every note) that's injected each session, so the agent picks from a known list instead of guessing keywords or hoping a vector match returns the right chunk; if a note isn't in the overview, the agent knows it doesn't exist.
