...
The "current project" for tests is `example-project`. `other-project-a` and
`other-project-b` exist only so the overview includes a non-empty
"Other projects" name list (a real-world signal).
