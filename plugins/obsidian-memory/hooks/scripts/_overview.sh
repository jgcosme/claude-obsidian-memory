...
#
# Usage: _overview.sh <vault_path> <project_name> [<project_vault_path>]
...
# Cache key includes the project-vault path so different projects get distinct
# cache entries. Empty project-vault still hashes uniquely from the vault+project.
KEY=$(printf '%s|%s|%s' "$VAULT" "$PROJECT" "$PROJECT_VAULT" | \
...
if [ -s "$CACHE" ]; then
  ...
  if [ -z "$newer" ] && [ -n "$PROJECT_VAULT" ] && [ -d "$PROJECT_VAULT" ]; then
    newer=$(find "$PROJECT_VAULT" -name '*.md' -newer "$CACHE" -print -quit 2>/dev/null)
  fi
  if [ -z "$newer" ]; then
    cat "$CACHE"
    exit 0
...
if [ -n "$PROJECT_VAULT" ] && [ -d "$PROJECT_VAULT" ]; then
  python3 "$VAULT_PY" --vault "$VAULT" overview --project "$PROJECT" --project-vault "$PROJECT_VAULT" > "$TMP" 2>/dev/null
else
