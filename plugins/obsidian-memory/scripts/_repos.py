#!/usr/bin/env python3
"""Repo-vault registry for the obsidian-memory plugin.

Manages ~/.config/obsidian-memory/repos.json — the single source of truth
for which project repos are registered as repo-vaults and whether they're
enabled. Used by:
  - session-start.sh    → look up cwd's status, prompt or eager-init
  - statusline.py       → append `• <project>` when registered+enabled
  - init_repo_vault.py  → write enabled/disabled entry after init
  - save-memory         → route writes when project has repo-vault
...
CLI:
  _repos.py lookup <path>            # prints "enabled"/"disabled"/"not_registered"
...
def repos_path() -> Path:
    """Resolve the repos.json path. Honors $OBSIDIAN_MEMORY_REPOS_FILE
    for tests; otherwise ~/.config/obsidian-memory/repos.json."""
    override = os.environ.get("OBSIDIAN_MEMORY_REPOS_FILE")
    if override:
        return Path(override).expanduser().resolve()
    return Path.home() / ".config/obsidian-memory/repos.json"
...
def register(path: str, *, enabled: bool, project: str) -> dict:
    ...
    rp = repos_path()
    data = _load(rp)
    data["repos"][repo_path] = {"enabled": enabled, "project": project}
    _save(rp, data)
    return {
        "status": "enabled" if enabled else "disabled",
        "path": repo_path,
...
def list_all() -> list[dict]:
    data = _load(repos_path())
    out: list[dict] = []
    for path, entry in sorted(data["repos"].items()):
        enabled = bool(entry.get("enabled", False))
        out.append({
            "path": path,
            "enabled": enabled,
...
def main(argv: list[str] | None = None) -> int:
    ...
    lp = sub.add_parser("lookup", help="check registration status of a repo path")
    lp.add_argument("path")
    lp.add_argument("--json", action="store_true")
    lp.set_defaults(func=_cmd_lookup)

    rp = sub.add_parser("register", help="add or update a repo entry")
    rp.add_argument("path")
    rp.add_argument("--enabled", dest="enabled", action="store_true",
                    help="mark this repo as enabled (default)")
    rp.add_argument("--no-enabled", dest="enabled", action="store_false",
                    help="mark this repo as disabled (declined)")
    ...
    pp = sub.add_parser("path", help="print the resolved repos.json path")
